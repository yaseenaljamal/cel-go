...
func PruneAst(expr ast.Expr, macroCalls map[int64]ast.Expr, state EvalState) *ast.AST {
	...
	pruner.ExprFactory = ast.NewExprFactory()
	newExpr, _ := pruner.maybePrune(expr)
	newAST := ast.NewAST(newExpr, sourceInfo)
	for id, e := range pruner.macroCalls {
		sourceInfo.SetMacroCall(id, e)
	}
	return newAST
}
...
func (p *astPruner) prune(node ast.Expr) (ast.Expr, bool) {
	...
	case ast.StructKind:
		...
		for i, field := range fields {
			...
			structPruned = true
			newEntry := p.NewStructField(field.ID(), f.FieldName(), newValue, f.IsOptional())
