...
func (p *planner) Plan(expr ast.Expr) (Interpretable, error) {
	...
	case ast.StructKind:
		return p.decorate(p.planCreateObj(expr))
...
func (p *planner) planCreateObj(expr ast.Expr) (Interpretable, error) {
	obj := expr.AsStruct()
	...
	for i, f := range entries {
		...
		fields[i] = entry.FieldName()
		val, err := p.Plan(entry.Value())
		if err != nil {
			return nil, err
		}
		vals[i] = val
