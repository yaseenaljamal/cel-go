...
func (e astNode) ComputedSize() *SizeEstimate {
	...
	case ast.LiteralKind:
		switch val := e.expr.AsLiteral().(type) {
		case types.String:
			// converting to runes here is an O(n) operation, but
			// this is consistent with how size is computed at runtime,
			// and how the language definition defines string size
			v = uint64(len([]rune(val)))
		case types.Bytes:
			v = uint64(len(val))
	...
	case ast.StructKind:
		v = uint64(len(e.expr.AsStruct().Fields()))
	default:
		return nil
	}
...
func (c *coster) costIdent(e ast.Expr) CostEstimate {
	...
	if iterRange, ok := c.iterRanges.peek(identExpr); ok {
...
func (c *coster) costSelect(e ast.Expr) CostEstimate {
	...
	sum = sum.Add(c.cost(sel.Operand()))
	targetType := c.checkedAST.GetType(sel.Operand().ID())
