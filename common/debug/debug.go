...
func (w *debugWriter) appendMap(obj ast.MapExpr) {
	w.append("{")
...
func formatLiteral(val ref.Val) string {
	switch val := val.(type) {
	case types.Bool:
		return fmt.Sprintf("%t", val)
	case types.Bytes:
		return fmt.Sprintf("b\"%s\"", string(val))
	case types.Double:
