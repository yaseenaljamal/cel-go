...
// CheckedASTToProto converts a CheckedAST to a CheckedExpr protobouf.
func ASTToProto(ast *AST) (*exprpb.CheckedExpr, error) {
...
// ProtoToCheckedAST converts a CheckedExpr protobuf to a CheckedAST instance.
func ProtoToAST(checked *exprpb.CheckedExpr) (*AST, error) {
...
func protoStructField(id int64, s StructField) (*exprpb.Expr_CreateStruct_Entry, error) {
	v, err := ExprToProto(s.Value())
	if err != nil {
		return nil, err
	}
	return &exprpb.Expr_CreateStruct_Entry{
		...
		KeyKind: &exprpb.Expr_CreateStruct_Entry_FieldKey{
			FieldKey: s.FieldName(),
		},
		Value:         v,
		OptionalEntry: s.IsOptional(),
	}, nil
}

func SourceInfoToProto(info *SourceInfo) (*exprpb.SourceInfo, error) {
	sourceInfo := &exprpb.SourceInfo{
		SyntaxVersion: info.syntax,
		Location:      info.desc,
		LineOffsets:   info.lines,
...
func ProtoToSourceInfo(info *exprpb.SourceInfo) (*SourceInfo, error) {
	...
	for id, offset := range info.GetPositions() {
		sourceInfo.offsetRanges[id] = OffsetRange{Start: offset, Stop: offset}
	}
